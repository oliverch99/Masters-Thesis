...
def get_thirtyMinuteWinner(ticker, postDateTime):
    ...
    # Get the trades for this section
    trades = alpaca.get_trades_by_time(ticker, startTime.strftime("%d/%m/%Y %H:%M"), endTime.strftime("%d/%m/%Y %H:%M"))
    #trades = get_trades_from_dataframe()
    if len(trades) < 3:
        return False
...
def collect_daily_data(df, from_date = '01/11/2021', to_date = '1/1/2022'):
    """
    Will collect, process, and store data for the given ticker and day in database
    :param ticker: stock ticker to process
    :return: none
    """
    days = alpaca.get_all_trading_days(from_date, to_date)
    ...
    for dayDate in days:
        print(dayDate)
        for _,submission in wsb_reddit.get_wsb_posts_for_day(df, dayDate).iterrows():
            #print(submission.score)
            tickers = wsb_reddit.get_tickers_from_title(str(submission.title), valid_tickers)
            for ticker in tickers:
                p = wsb_post.WSBPost()
                p.postId = submission.id
                p.ticker = ticker.upper()
                p.title = str(submission.title)
                p.title = p.title.replace("\'", "")
                p.title = p.title.replace("\"", "")
                p.description = str(submission.selftext)
                p.description = p.description.replace("\'", "")
                p.description = p.description.replace("\"", "")
                p.postTime = submission.created
                # Check for winners
                #adjustedDatetime = datetime.fromtimestamp(p.postTime)
                adjustedDatetime = datetime.strptime(p.postTime, "%Y-%m-%d %H:%M")
...
#collect_daily_data(df = read_csv("wallstreetbets_submissions.csv"), from_date = '11/04/2016', to_date = '01/01/2024')

#collect_daily_data(df = read_csv("wallstreetbets_submissions.csv"), from_date = '06/04/2022', to_date = '01/01/2024')