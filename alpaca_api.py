...
class Alpaca:
    ...
    def get_all_trading_days(self, startDate, endDate):
        ...
        try:
            ...
            for r in response:
                ...
                datesList.append(day.strftime('%Y-%m-%d'))
            return datesList
        except Exception as e:
            print(e)
            return []
    ...
    def get_minute_bars_by_time(self, ticker, strStartTime, strEndTime):
        ...
        try:
            ...
            endTime = datetime.strptime(strEndTime, "%Y-%m-%d %H:%M")
        except Exception as e:
        ...
        try:
            bars = self.api.get_bars(symbol=ticker,
                                     ...
                                     adjustment='raw')
            bars = pd.DataFrame([{
            'time': bar.t,  # Assuming 't' is the attribute for time
            'open': bar.o,  # Assuming 'o' is the attribute for open price
            'high': bar.h,  # Assuming 'h' is the attribute for high price
            'low': bar.l,   # Assuming 'l' is the attribute for low price
            'close': bar.c,  # Assuming 'c' is the attribute for close price
            'volume': bar.v,  # Assuming 'v' is the attribute for volume
            'num_trades': bar.n,  # Assuming 'n' is the attribute for number of trades
            'vwap': bar.vw  # Assuming 'vw' is the attribute for volume-weighted average price
            } for bar in bars])
    ...
    def get_trades_by_time(self, ticker, strStartTime, strEndTime):
        ...
        try:
            startTime = datetime.strptime(strStartTime, "%Y-%m-%d %H:%M")
            totalEndTime = datetime.strptime(strEndTime, "%Y-%m-%d %H:%M")
        except Exception as e:
            print(e, ':: date could not be converted from input values')
            return {}
